...
function randomFrom<T>(items: readonly T[], seed: string): T {
  const index = Math.abs(seed.split("").reduce((total, character) => total + (character.codePointAt(0) ?? 0), 0)) % items.length;
  return items[index];
}
  ...
  try {
    ...
    const { data: userData, error: createError } = await admin.auth.admin.createUser({
      ...
      user_metadata: {
        ...
        accent_color: randomFrom(ACCENT_CHOICES, username),
        wallpaper: randomFrom(WALLPAPER_CHOICES, `${username}w`),
      },
    });
