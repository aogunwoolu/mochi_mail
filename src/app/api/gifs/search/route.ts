...
export async function GET(req: NextRequest) {
  ...
  if (provider === "gifapi") {
    ...
    if (!key) return NextResponse.json({ error: "GifAPI key not configured in environment variables" }, { status: 503 });
    const url = `${GIFAPI_BASE}?q=${encodeURIComponent(q)}&limit=18&api_key=${key}`;
    const res = await fetch(url);
    const json = await res.json();
    if (!res.ok) return NextResponse.json({ error: `GifAPI search failed: ${res.statusText}` }, { status: 502 });
    return NextResponse.json(json);
  }

  // default: giphy
  const key = process.env.GIPHY_API_KEY;
  if (!key) return NextResponse.json({ error: "Giphy API key not configured in environment variables" }, { status: 503 });
  const url = `${GIPHY_BASE}?q=${encodeURIComponent(q)}&limit=18&rating=g&lang=en&api_key=${key}`;
  const res = await fetch(url);
  const json = await res.json();
  if (!res.ok) return NextResponse.json({ error: `Giphy search failed: ${res.statusText}` }, { status: 502 });
  return NextResponse.json(json);
}
