...
function RoomsPageInner() {
  ...
  const inviteFromUrl = new URLSearchParams(searchString).get("invite")?.trim() ?? "";
    ...
    return (
      <div className="flex h-svh items-center justify-center p-4">
        <div className="panel max-w-sm rounded-3xl p-6 text-center">
          ...
          <button onClick={() => navigate("/")} className="btn-smooth mt-4 rounded-xl px-4 py-2 text-xs font-semibold text-white" style={{ background: "linear-gradient(135deg, var(--pink), var(--lavender))" }}>
            Back to studio
          </button>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto flex min-h-svh w-full max-w-2xl flex-col gap-5 p-4 pb-10">
      ...
      <header className="flex items-center justify-between">
        <button onClick={() => navigate("/")} className="btn-smooth flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-semibold" style={{ background: "var(--surface)", color: "var(--muted-strong)" }}>
      ...
      {primaryRoom ? (
        <section className="rounded-2xl border p-5 flex flex-col gap-4" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              onClick={() => navigate(`/?room=${encodeURIComponent(primaryRoom.inviteToken)}`)}
          ...
          <div className="flex flex-col gap-2">
            {otherRooms.map((room) => (
              <RoomRow
                key={room.id}
                room={room}
                onOpen={() => navigate(`/?room=${encodeURIComponent(room.inviteToken)}`)}
                onCopyLink={async () => {
